...
import { Form, TextField, Button, ButtonGroup, DatePicker } from "@adobe/react-spectrum";
...
const PaymentForm = ({
    ...
    const handleSave = async () => {
        try {
            const url = "http://127.0.0.1:5000/payments/add";
            ...
            if (!response.ok) {
                ...
                ToastQueue.negative(errorMessage.error || "Failed to save payment", { timeout: 3000 });
                throw new Error(errorMessage.error || "Failed to save payment");
            }

            const responseMessage = await response.json();
            ToastQueue.positive(responseMessage.message || "Payment saved successfully", { timeout: 3000 });
            onPaymentAdded();
            close();
        } catch (error) {
